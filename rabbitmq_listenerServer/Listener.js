...
async function convertToCoord(address) {
  try {
    ...
    return await axios.get(
      `https://rsapi.goong.io/geocode?address=${searchText}&api_key=NdHC0NcH8Om0kgYqIwrmdALpmlANLlST3CLWW45M`
    );
  } catch (error) {
    console.log(error);
  }
...
async function consumeMessages() {
  const connection = await amqp.connect(
    "amqps://qgkfypdz:bpDTputLB1EzSTiB0N5Of_OvINBuX8Yq@armadillo.rmq.cloudamqp.com/qgkfypdz"
  );
